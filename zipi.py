...
############### FII14 ###############
{r}""")
file_zip = input(f"{c}[»] Masukan nama file zip: ")
if(os.path.isfile(file_zip)):
    if file_zip.endswith(".zip"):
        time.sleep(1)
        file_wordlist = input(f"{c}[»] Masukan nama file wordlist: ")
                ...
                with open(file_wordlist, "r") as fw:
                    for baris in fw:
                        ...
                        try:
                            ...
                            time.sleep(1)
                            print(f"{p}[{h}INFO{p}] Kata sandi ditemukan: {kata_sandi}{r}\n")
                            break
                        except:
                            time.sleep(1)
                            print(f"{k}[!] Mencoba kata sandi: {kata_sandi}{r}")
                    else:
                        print(f"{p}[{m}INFO{p}] Kata sandi tidak ditemukan dalam file wordlist.{r}\n")
        else:
            time.sleep(3)
